...
var voca05 = Paper.image('img/voca05.png', 0, 0, 130, 120).toDefs();
...
voca05.use().transform('t113, 95').click(playAud01).appendTo(topArea).attr({
  'cursor': 'pointer'
});

var t1 = topArea.text(180, 300, ['おくじょう', '옥상 (屋上)']).click(playAud01).attr({
  'font-size': 40,
  'text-anchor': 'middle',
  'cursor': 'pointer'
});
...
bottomArea.text(180, 462, '옥상은 깨끗했습니다.').attr({
  'font-size': 18,
  'text-anchor': 'middle'
});
...
function handler() {
  ...
  var t3 = bottomArea.text(185, 540, ['おくじょう', 'は', ' きれい', 'でした。']).attr({
    'font-size': 25,
    'fill': 'gray',
    'text-anchor': 'middle'
  });
  ...
  // var t4 = bottomArea.text(185, 560, ['きれい', 'でした。']).attr({
  //   'font-size': 25,
  //   'fill': 'gray',
  //   'text-anchor': 'middle',
  //   'letter-spacing': 1.2
