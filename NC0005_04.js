...
var voca04 = Paper.image('img/voca04.png', 0, 0, 130, 120).toDefs();
...
function handlerNext() {
  location.replace('NC0005_05.html');
}

// 상단
var topArea = Paper.g();

voca04.use().transform('t113, 95').click(playAud01).appendTo(topArea).attr({
  'cursor': 'pointer'
});

var t1 = topArea.text(180, 300, ['ろうか', '복도 (廊下)']).click(playAud01).attr({
  'font-size': 40,
  'text-anchor': 'middle',
  'cursor': 'pointer'
});
...
bottomArea.text(180, 462, '복도는 깨끗했습니까?').attr({
  'font-size': 18,
  'text-anchor': 'middle'
});
...
function handler() {
  ...
  var t3 = bottomArea.text(185, 540, ['ろうか', 'は', ' きれい', 'でしたか。']).attr({
    'font-size': 25,
    'fill': 'gray',
    'text-anchor': 'middle'
  });
