...
var voca03 = Paper.image('img/voca03.png', 0, 0, 130, 120).toDefs();
...
function handlerNext() {
  location.replace('NC0005_04.html');
}

// 상단
var topArea = Paper.g();

voca03.use().transform('t113, 95').click(playAud01).appendTo(topArea).attr({
  'cursor': 'pointer'
});

var t1 = topArea.text(180, 300, ['ベランダ', '베란다 (veranda)']).click(playAud01).attr({
  'font-size': 40,
  'text-anchor': 'middle',
  'cursor': 'pointer'
});
...
bottomArea.text(180, 462, '베란다는 깨끗했습니다.').attr({
  'font-size': 18,
  'text-anchor': 'middle'
});
...
function handler() {
  ...
  var t3 = bottomArea.text(185, 540, ['ベランダ', 'は', ' きれい', 'でした。']).attr({
    'font-size': 25,
    'fill': 'gray',
    'text-anchor': 'middle'
  });
